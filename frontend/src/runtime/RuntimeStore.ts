...
export interface Decision2DisplayRecord { timestampMs: number; message: string }
...
export interface RuntimeStoreState {
  ...
  decision2History: readonly Decision2DisplayRecord[];
  ...
  recordDecision2: (value: Decision2DisplayRecord) => void;
...
export const createRuntimeStore = () => createStore<RuntimeStoreState>((set, get) => ({
  ...
  connectionState: { status: 'disconnected', attempt: 0, latencyMs: null, lastMessageAtMs: null }, decision2History: [],
  ...
  recordDecision2: (value) => set((state) => ({ decision2History: [...state.decision2History, Object.freeze({ ...value })] })),
  ...
  resetSessionStreams: () => set({ neuroState: null, neuroStateReceivedAtMs: null, sceneJourneyPlan: null, sceneJourneyPlanReceivedAtMs: null, runtimeWorldState: null, runtimeValidationErrors: [], decision2History: [], sessionRuntime: { elapsedTimeMs: 0, status: 'idle', plannerStatus: 'idle' } }),
}));
