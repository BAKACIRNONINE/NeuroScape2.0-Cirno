...
export interface SummaryMetrics { averageAttention: number | null; averageArousal: number | null; averageStability: number | null; averageConfidence: number | null; durationMs: number; journeyDistance: number; locationsVisited: string[]; eventCount: number }
...
export function deriveSummary(recording: RecordedSession): SummaryMetrics {
  ...
  return { averageAttention: average(neuro.map((item) => item.attention.value)), averageArousal: average(neuro.map((item) => item.arousal.value)), averageStability: average(neuro.map((item) => item.stability)), averageConfidence: average(neuro.map((item) => item.confidence)), durationMs: recording.metadata.durationMs, journeyDistance: journeyDistance(runtime), locationsVisited: locations, eventCount };
}
...
export function descriptiveReflection(recording: RecordedSession): string {
  ...
  const midpoint = Math.ceil(recording.neuroStates.length / 2), first = average(recording.neuroStates.slice(0, midpoint).map((item) => item.attention.value))!, second = average(recording.neuroStates.slice(midpoint).map((item) => item.attention.value)) ?? first;
  ...
  return `Attention was ${direction} in the latter portion of the recorded session${location ? ` while the listener journey ended at ${location}` : ''}. This is a descriptive coincidence, not a causal interpretation.`;
}
