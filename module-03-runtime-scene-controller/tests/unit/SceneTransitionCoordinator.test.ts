...
describe('SceneTransitionCoordinator', () => {
  ...
  it('cancels stale transition coordination without resetting time', () => {
    const events = new RuntimeEventBus();
    const coordinator = new SceneTransitionCoordinator(events);
    ...
    coordinator.update(
      5_000,
      listener('forest_clearing', [0, 0, -1]),
    );
    ...
    const restarted = coordinator.start({
      fromLocationId: 'forest_clearing',
      toLocationId: 'dense_forest',
      arrivalTimeMs: 31_000,
    });

    expect(restarted?.startedAtMs).toBe(6_000);
  });
